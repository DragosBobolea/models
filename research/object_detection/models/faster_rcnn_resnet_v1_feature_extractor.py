      ...
      with slim.arg_scope(
        ...
        with tf.variable_scope(
          ...
          _, activations = self._resnet_model(
              ...
              scope=var_scope)
    activations_number = self._feature_count
    feature_activations = []
    use_top_lstm = False
        
    for index in range(activations_number):        
        activation_index = 3 - index
        #handle = scope + '/%s/block3' % self._architecture
        block_index = "/%s/block"+str(activation_index)
        handle = scope + block_index % self._architecture
        if index == 0 and use_top_lstm == True:
            act = activations[handle]
            act =  lstm2d.separable_lstm(act, 512, kernel_size=None, nhidden=None, scope='lstm2d-1')
            feature_activations.append(act)
        else:
            feature_activations.append(activations[handle])
    ...
    with tf.variable_scope(self._architecture, reuse=self._reuse_weights):
      with slim.arg_scope(
        ...
        with slim.arg_scope([slim.batch_norm],
                            is_training=self._train_batch_norm):
          depth = proposal_feature_maps.shape[3].value
          if depth != 1024:          
            blocks = [
                 resnet_utils.Block('block4a', resnet_v1.bottleneck, [{
                     'depth': 2048,
...
class FasterRCNNResnet50FeatureExtractor(FasterRCNNResnetV1FeatureExtractor):
  ...
  def __init__(self,
    ...
    super(FasterRCNNResnet50FeatureExtractor, self).__init__(
        ...
        reuse_weights, weight_decay, use_fpn, feature_count)
